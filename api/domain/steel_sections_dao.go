...
type steelSectionsDaoInterface interface {
	GetSteelSections() (*[]SteelSections, error)
	Create(steelSectionsName string, DbValue string) error
	SetClient()
}

type steelSectionsDao struct {
	client *sqlx.DB
}

var (
	SteelSectionsDao steelSectionsDaoInterface
)

func init() {
	SteelSectionsDao = &steelSectionsDao{}
	SteelSectionsDao.SetClient()
}

func (s *steelSectionsDao) SetClient() {
	s.client = utils.GetMYSQLConnection()
}

func (s *steelSectionsDao) Create(steelSectionsName string, DbValue string) error {
	sql := `INSERT INTO steelSections (steelSectionsName, db_value) VALUES (?, ?)`

	_, err := s.client.Exec(sql, steelSectionsName, DbValue)
	return err
}

func (s *steelSectionsDao) GetSteelSections() (*[]SteelSections, error) {
	var steelSections []SteelSections
	sql := `SELECT 
				idsteel_sections AS id,
				steel_sections_shape AS steel_sections_shape,
       			steel_sections_type AS steel_sections_type,
       			steel_sections_aisc_manual_label_english AS steel_sections_aisc_manual_label_english,
       			steel_sections_aisc_manual_label_metric AS steel_sections_aisc_manual_label_metric,
			...
			FROM steel_sections`
	err := s.client.Select(&steelSections, sql)
	return &steelSections, err
}
